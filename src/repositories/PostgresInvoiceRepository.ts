...
export class PostgresInvoiceRepository implements IInvoiceRepository {
  ...
  async save(invoice: Invoice): Promise<Invoice> {
    await this.pool.query(
      `INSERT INTO invoices (id, client_cif, client_name, client_address, base_amount, vat_amount, total_amount, invoice_number, status, created_at, finalized_at)
  ...
  private mapRowToInvoice(row: Record<string, unknown>): Invoice {
    return {
      id: row.id as string,
      clientCif: row.client_cif as string,
      clientName: row.client_name as string,
      clientAddress: row.client_address as string,
      baseAmount: parseFloat(row.base_amount as string),
      vatAmount: parseFloat(row.vat_amount as string),
